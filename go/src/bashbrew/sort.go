...
func sortRepoObjects(rs []*Repo, applyConstraints bool) ([]*Repo, error) {
	...
	for _, r := range rs {
		...
		for _, entry := range r.Entries() {
			...
			// this should be reasonably harmless for other use cases of --namespace but catches things like "docker-pkg/foo -> docker-pkg/bar" and things like "php -> wordpress" equally even if we're building to target a different namespace
			tags := []string{}
